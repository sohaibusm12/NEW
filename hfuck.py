...
with open("premiumproxies.txt") as fp:
    proxies = fp.read().splitlines()
ptype = config['bypassproxiestype']
...
def REQ_Data(host, sitekey, proxy):
        ...
        try:
            proxyServer = f'{ptype}://{proxy}'
            r = requests.get(f"https://hcaptcha.com/checksiteconfig?host={host}&sitekey={sitekey}&sc=1&swa=1", headers=headers,  timeout=4, proxies={"http://": proxyServer, "https://": proxyServer})
            if r.json()["pass"]:
                return r.json()["c"]
            else:
                return False
...
def Get_Captcha(host, sitekey, n, req, proxy):
        try:
            ...
            r = requests.post(f"https://hcaptcha.com/getcaptcha?s={sitekey}",cookies=cookies, data=data, headers=headers, timeout=4, proxies={"http://": proxyServer, "https://": proxyServer}) # proxies={"http://": proxyServer, "https://": proxyServer}
            return r.json()
        except Exception as e:
            print(e)
            return False
...
if __name__ == '__main__':
	print(bypass("f5561ba9-8f1e-40ca-9b5b-a0b3f719ef34", "discord.com", '', 's'))